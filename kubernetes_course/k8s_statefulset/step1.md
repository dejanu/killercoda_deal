...
Kubernetes looks for volumeClaimTemplates entry in the StatefulSet spec, without it the pods will be failed to start: <code>spec.containers[0].volumeMounts[0].name: Not found: "postgres-storage"</code>. 

Compare <code>postgres-statefulset.yaml</code> with <code>postgres-good-statefulset.yaml</code> and find the missing part.

Try to create again the sts with <code>kubectl apply -f postgres-good-statefulset.yaml</code> 
<br>
