...
```
<br>
<b>last-applied-configuration</b> annotation is used by <code>kubectl apply</code> (client side by default) to store the previous configuration of the object, to avoid conflicts and to calculate the diff between the last applied configuration and the current state of the object. 
...
Explicitly instructs kubectl to include the field ownership tracking metadata (metadata.managedFields) in the YAML output: `kubectl get deploy api -n playground -o yaml --show-managed-fields`
