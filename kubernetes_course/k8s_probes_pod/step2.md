...
Delete the latter pod and create a new nginxpod with **liveness probe**. Create the `pod.yaml` manifest and `kubectl apply -f pod.yaml`{{copy}}. 
...
What happens? Inspect the pod: `kubectl describe po nginx`{{copy}}
